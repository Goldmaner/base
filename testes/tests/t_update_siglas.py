...
print("=== Verificando estrutura da tabela c_geral_tipo_contrato ===")
cur.execute("""
    SELECT column_name, data_type 
    FROM information_schema.columns 
    WHERE table_name = 'c_geral_tipo_contrato'
...
if not tem_sigla:
    ...
    cur.execute("ALTER TABLE c_geral_tipo_contrato ADD COLUMN sigla VARCHAR(10)")
    conn.commit()
...
cur.execute("SELECT * FROM categoricas.c_geral_tipo_contrato ORDER BY id")
rows = cur.fetchall()
for row in rows:
...
for informacao, sigla in mapeamento.items():
    cur.execute("""
        UPDATE categoricas.c_geral_tipo_contrato 
        SET sigla = %s 
        WHERE informacao = %s
    """, (sigla, informacao))
    if cur.rowcount > 0:
...
cur.execute("SELECT * FROM categoricas.c_geral_tipo_contrato ORDER BY sigla")
rows = cur.fetchall()
for row in rows:
