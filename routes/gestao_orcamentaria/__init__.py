...
# ---------------------------------------------------------------------------
# FUMCAD — Resultados / Disponibilidade
# ---------------------------------------------------------------------------

@gestao_orcamentaria_bp.route('/relatorio-fumcad')
@login_required
@requires_access('gestao_orcamentaria')
def relatorio_fumcad():
    """Página de Resultados FUMCAD"""
    return render_template('gestao_orcamentaria/relatorio_fumcad.html')


@gestao_orcamentaria_bp.route('/api/fumcad/disponibilidade')
@login_required
@requires_access('gestao_orcamentaria')
def api_fumcad_disponibilidade():
    """
    Retorna os valores indisponíveis do FUMCAD agrupados por tipo (TFM / TCV).

    Critérios de inclusão:
        - numero_termo ILIKE '%FUMCAD%'
        - parcela_tipo IN ('Programada', 'Projetada')
        - parcela_status IN ('Encaminhado para Pagamento', 'Não Pago')
        - parcela_status_secundario vazio ou '-'
        - data_assinatura da linha base em parcerias_sei (aditamento='-' e apostilamento='-')
          deve ser <= último dia do mês seguinte ao selecionado (ou NULL = sem restrição)
    ...
    try:
        ...
        cur.execute("""
            ...
            WHERE
                ...
                -- Excluir termos assinados após o último dia do mês seguinte ao selecionado
                AND (
        ...
        resultado = {'TFM': 0.0, 'TCV': 0.0}
        for row in rows:
            if row['tipo'] in resultado:
                resultado[row['tipo']] = float(row['valor_indisponivel'] or 0)

        return jsonify({
            'success': True,
            'mes': mes,
            'ano': ano,
            'termo_fomento': resultado['TFM'],
            'termo_convenio': resultado['TCV'],
        })
