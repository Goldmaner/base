"""
RLS - calendario (todas as tabelas)
Padrão: authenticated → ALL | anon → bloqueado
Tabelas sem RLS habilitado recebem ALTER TABLE ... ENABLE ROW LEVEL SECURITY primeiro.
"""
import psycopg2, os
...
cur.execute("""
    SELECT t.tablename, t.rowsecurity,
           ...
           array_agg(p.roles      ORDER BY p.policyname) FILTER (WHERE p.policyname IS NOT NULL) AS roles
    FROM pg_tables t
...
for r in tabelas_info:
    ...
    try:
        ...
        # Habilita RLS se ainda desabilitado
