"""
RLS - public (todas as tabelas da aplicação)
Padrão: authenticated → ALL | anon → bloqueado
Ignora tabelas de sistema do Supabase que possam existir no schema public.
"""
import psycopg2, os
...
# Tabelas do sistema Supabase/PostgREST que não devem ser tocadas
IGNORAR = {'schema_migrations', 'ar_internal_metadata'}
...
""", (SCHEMA,))
tabelas_info = [r for r in cur.fetchall() if r['tablename'] not in IGNORAR]
...
for r in tabelas_info:
    ...
    try:
        ...
        cur.execute(f"""
            ...
            ON {full} FOR ALL TO authenticated
...
print(f"\n=== RESULTADO ===")
print(f"\n✓ Aplicadas ({len(ok)}):")
for t in ok: print(f"  {t}")
if puladas:
    print(f"\n— Já corretas ({len(puladas)}):")
    for t in puladas: print(f"  {t}")
if erros:
    print(f"\n✗ Erros ({len(erros)}):")
