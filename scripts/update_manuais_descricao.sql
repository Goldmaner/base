-- ============================================================
-- UPDATE: public.manuais_lista — manual_descricao
-- Fonte: Lista de Operações do Departamento de Parcerias
-- ============================================================
...
Objetivo: criação ou aprimoramento de ferramentas, fluxos e procedimentos, visando maior eficiência, clareza e uniformidade técnica, consistindo em avaliar periodicamente os processos internos, identificar fragilidades, propor ajustes e desenvolver metodologias que sustentem a execução das demais operações.

Também envolve elaboração de modelos de documentos e manuais, até a definição de padrões técnicos e indicadores de desempenho, assegurando coerência e alinhamento às diretrizes institucionais.'
WHERE manual_nome = 'Análise e desenvolvimento de metodologias'
...
-- ── Verificação ───────────────────────────────────────────
SELECT manual_area, manual_nome,
       CASE WHEN manual_descricao IS NULL THEN '*** SEM DESCRIÇÃO ***'
            ELSE LEFT(manual_descricao, 60) || '...'
       END AS descricao_preview
FROM public.manuais_lista
ORDER BY manual_area, manual_nome;

UPDATE public.manuais_lista SET manual_descricao =
'[Preencher com a descrição do documento Lista_Operações.docx]'
WHERE manual_nome = 'Relatórios e assessoramento'
  AND manual_area = 'Operações Gerais da Unidade (Compartilhadas)';
